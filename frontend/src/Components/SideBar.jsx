    ...
    <div className='flex flex-col justify-between bg-white h-full overflow-y-scroll min-w-210 hide-scrollbar'>
      <div className='flex flex-col'>
        <Link to='/' className='flex px-5 gap-2 my-6 pt-1 w-190 items-center' onClick={handleCloseSidebar}>
          <h2 className='w-full font-bold	'>SHARE_HERE</h2>
        </Link>
        <div className='flex flex-col gap-5'>
          ...
          {Categories.slice(0, Categories.length - 1).map((category, index) => (
            <NavLink to={`/category/${category.name}`} className={({ isActive }) => isActive ? isActiveStyle : isNotActiveStyle} onClick={handleCloseSidebar} key={index}>
              {category.name}
            </NavLink>
          ))}
        </div>
